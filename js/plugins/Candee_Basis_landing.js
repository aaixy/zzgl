 ...
 * @param Data_ActorsAndRobots
 * @desc 添加既可以用来乘降又可以作为伙伴出现的角色，有多个时用‘,’隔开
 * @default 37,62,63,64,65
 * @param Sitdown_name
 * @desc 乘战车的名称
 * @default 乘战车
 * @param Standup_name
 ...
 *         修改事件启动判断中角色是否在队判断
 * 20190206
 * 	增加变量用来存放既可以作为战车又可以作为伙伴的角色id: Data_ActorsAndRobots;
 */
...
(function() {
	...
	var actorsAndRobotsData = String(parameters['Data_ActorsAndRobots'] || [37,62,63,64,65]);
    ...
    if (!this._actors.contains(actorId)) {
		...
		if (data.indexOf(actorId)!=-1){
			    ...
			    $gameVariables.value(variablesData)[actorId]==0)
			{
				...
				//if(!data.contains(mecha._actorId)){//既是可乘坐战车又是伙伴的赋予战车收纳标志同时让其显式入队20190206
				//console.log(actorId);
				if(actorsAndRobotsData.split(',').contains(actorId.toString())){
			...
			try{
				...
				if (!data1.contains(actorId)){this._actors.push(actorId)}//车上不含有的情况(离队之前乘坐了战车？)下添加角色
			}catch(e){
		...
		if (data2.contains(actorId)){//如果该角色乘坐了战车
			...
			for (id of data3){
				if ($gameVariables.value(variablesData)[id]!=null){
					...
					//当该角色在index所指的战车上时
					if (index>-1){
						$gameVariables.value(variablesData)[id].splice(index, 1);
						if ($gameVariables.value(variablesData)[id].length==0){//战车没人乘坐时收入异空间
							//this._actors.splice(this._actors.indexOf(id), 1);
							//if(!data.contains(mecha._actorId)){//既是可乘坐战车又是伙伴的就不放入异空间，否则放入异空间，也就是用splice方法使其离队20190206
							if(!actorsAndRobotsData.split(',').contains(id.toString())){
								this._actors.splice(this._actors.indexOf(id), 1);
							}
						} //战车没人乘坐
					}
...
Window_Basis_LandingActor.prototype.onOk = function(mecha) {
	//var data = [];
	//for (i of actorsAndRobotsData.split(',').map(Number)){data.push(i)}
	//console.log(data);
	//console.log(actorsAndRobotsData.split(','));
	...
	switch (this._category) {
		...
		case 'Standup'://下战车的情况
			...
			if ($gameVariables.value(variablesData)[mecha_id].length==0){
				//if(!data.contains(mecha._actorId)){//既是可乘坐战车又是伙伴的就不放入异空间，否则放入异空间，也就是用splice方法使其离队20190206
				if(!actorsAndRobotsData.split(',').contains(mecha._actorId.toString())){
...
BattleManager.gainExp = function() {
	...
	console.log($gameVariables.value(variablesData));
	try{
		...
		$gameParty.allMembers().forEach(function(actor) {
			if (data.contains(actor.actorId().toString())){//如果是战车，那么进入下一步
				//if(actorsAndRobotsData.split(',').contains(actor.actorId().toString())){//既是可乘坐战车又是伙伴的给经验20190206，注释段20190206134550
				//	candee_landing_gainexp=false;
				//	actor.gainExp(exp);
				//}
				if ($gameVariables.value(variablesData)[actor.actorId()]!=null && $gameVariables.value(variablesData)[actor.actorId()].length!=0){
					$gameVariables.value(variablesData)[actor.actorId()].forEach(function(id) {//有乘坐的依次给乘坐的角色添加经验
						candee_landing_gainexp=true;
						$gameActors.actor(id).gainExp(exp);
					});
					actor.gainExp(exp);//给被乘坐的战车也添加经验，数据中设置了获取经验为0，所以这里不影响，只是为了结算页面看到经验数据，注释句20190206134730
				}
				else{//貌似无法进入该分支20190206，实际测试与“注释段20190206134550”一个意思，并且”注释段20190206134550“与”注释句20190206134730“会导致在战车既可以乘坐又可以作为伙伴的时候重复获得经验，因此开放这里，注释掉”注释段20190206134550“。
					candee_landing_gainexp=false;
					actor.gainExp(exp);
				}
			}else{//不是战车直接给经验
